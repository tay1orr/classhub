...
export default function ExamBoardPage() {
  ...
  const loadPosts = async () => {
    try {
      ...
      if (response.ok) {
        // 지필평가 게시글만 필터링
        const examPosts = data.posts.filter((post: any) => post.board === 'exam')
        
        // 공지사항을 먼저, 그 다음 최신순으로 정렬
        examPosts.sort((a: any, b: any) => {
          // 공지사항 우선
        ...
        setPosts(examPosts)
        console.log('Exam posts loaded:', examPosts.length)
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-purple-600">📚 우리반 지필평가</h1>
          <p className="text-muted-foreground mt-2">
            시험 정보와 스터디 자료를 나누고 함께 공부해요
          </p>
        </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <Link href={`/1-8/exam/${post.id}`} onClick={(e) => isSelectMode && e.preventDefault()}>
