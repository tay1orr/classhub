...
export default function WriteAssignmentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('게시글이 등록되었습니다!')
      // 강제 새로고침으로 최신 데이터 로드
      window.location.href = `/1-8/assignment?refresh=${Date.now()}`
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/1-8/assignment" className="text-blue-600 hover:underline">
          ← 수행평가로 돌아가기
        </Link>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>새 수행평가 게시글 작성</CardTitle>
          <CardDescription>
            수행평가 관련 정보나 자료를 공유해주세요.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="title"
                type="text"
                placeholder="수행평가 게시글 제목을 입력하세요"
                value={title}
