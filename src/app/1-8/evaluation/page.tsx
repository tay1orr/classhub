...
export default function EvaluationBoardPage() {
  ...
  const loadPosts = async () => {
    try {
      ...
      if (response.ok) {
        // 평가 게시글만 필터링
        const evaluationPosts = data.posts.filter((post: any) => post.board === 'evaluation')
        
        // 공지사항을 먼저, 그 다음 최신순으로 정렬
        evaluationPosts.sort((a: any, b: any) => {
          // 공지사항 우선
        ...
        setPosts(evaluationPosts)
        console.log('Evaluation posts loaded:', evaluationPosts.length)
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-green-600">📝 우리반 수행/지필평가</h1>
          <p className="text-muted-foreground mt-2">
            과제 정보와 자료를 함께 공유하고 도움을 주고받아요
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 items-end">
          {/* 글쓰기 버튼 */}
          {user ? (
            <Link href="/1-8/evaluation/write">
        ...
        {filteredPosts.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <Link href={`/1-8/evaluation/${post.id}`} onClick={(e) => isSelectMode && e.preventDefault()}>
          ...
          <Card>
            <CardContent className="pt-6 text-center py-12">
              ...
              {user && (
                <Link href="/1-8/evaluation/write">
                  <Button className="bg-green-600 hover:bg-green-700">첫 게시글 작성하기</Button>
                </Link>
              )}
            </CardContent>
          </Card>
