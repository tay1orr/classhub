...
export default function SuggestionBoardPage() {
  ...
  const loadPosts = async () => {
    try {
      ...
      if (response.ok) {
        // 건의사항 게시글만 필터링
        const suggestionPosts = data.posts.filter((post: any) => post.board === 'suggestion')
        
        // 공지사항을 먼저, 그 다음 최신순으로 정렬
        suggestionPosts.sort((a: any, b: any) => {
          // 공지사항 우선
        ...
        setPosts(suggestionPosts)
        console.log('Suggestion posts loaded:', suggestionPosts.length)
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-purple-600">💡 우리반 건의사항</h1>
          <p className="text-muted-foreground mt-2">
            우리반을 위한 건의사항이나 개선 아이디어를 나누어 주세요
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 items-end">
          {/* 글쓰기 버튼 */}
          {user ? (
            <Link href="/1-8/suggestion/write">
        ...
        {filteredPosts.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <Link href={`/1-8/suggestion/${post.id}`} onClick={(e) => isSelectMode && e.preventDefault()}>
                          <h3 className={`text-lg font-semibold cursor-pointer flex items-center gap-2 ${!isSelectMode ? 'hover:text-purple-600' : 'text-gray-700'}`}>
                            {post.title}
                            {(post.comments || 0) > 0 && (
          ...
          <Card>
            <CardContent className="pt-6 text-center py-12">
              ...
              {user && (
                <Link href="/1-8/suggestion/write">
                  <Button className="bg-purple-600 hover:bg-purple-700">첫 게시글 작성하기</Button>
                </Link>
              )}
            </CardContent>
          </Card>
