...
export default function NavigationClient({ user }: { user: SessionUser | null }) {
  ...
  const handleLogout = async () => {
    ...
    clearSession() // localStorage도 함께 정리
    router.push('/')
    router.refresh()
  }
        ...
        <div className="flex h-16 items-center justify-between">
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                    active ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'
                  }`}>
                  {item.label}
                </Link>
              )
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <>
                <Link href="/login" className="px-4 py-1.5 text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors">로그인</Link>
                <Link href="/signup" className="px-4 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors">회원가입</Link>
              </>
            )}
          </div>
          ...
          <div className="border-t pt-3 mt-3">
            {user ? (
              ...
              <div className="flex gap-2">
                <Link href="/login" onClick={() => setMenuOpen(false)} className="flex-1 text-center px-4 py-2 text-sm border rounded-md hover:bg-gray-50">로그인</Link>
                <Link href="/signup" onClick={() => setMenuOpen(false)} className="flex-1 text-center px-4 py-2 text-sm text-white bg-blue-600 rounded-md hover:bg-blue-700">회원가입</Link>
              </div>
            )}
          </div>
        </div>
